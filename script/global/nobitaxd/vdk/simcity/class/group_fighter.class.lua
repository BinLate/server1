...
function GroupFighter:_addNpcGo(tbNpc, isNew, goX, goY)	
	...
	if nMapIndex >= 0 then
		...
		nNpcIndex = AddNpcEx(tbNpc.nNpcId, _spawnLv, tbNpc.series, nMapIndex, tX * 32, tY * 32, 1, name, 0)
		...
		if nNpcIndex > 0 then
			...
			if kind ~= 0 then 
				...
				if SetNpcLevel then SetNpcLevel(nNpcIndex, tbNpc.level or 1) end
				tbNpc.lastPos = {
					nX32 = tX*32,
					nY32 = tY*32,
					nPosId = nPosId
				...
				if SimGear and SimGear.ApplyGearStats then
					SimGear:ApplyGearStats(tbNpc, nNpcIndex)
				elseif tbNpc.cap and NPCINFO_SetNpcCurrentLife then
...
function GroupFighter:New(tbNpc)
	...
	if worldInfo.showName == 1 then
		if (not tbNpc.szName) or tbNpc.szName == "" then
			tbNpc.szName = SimCityNPCInfo:generateName() or SimCityNPCInfo:getName(id)
		end
	else
		tbNpc.szName = " "
	end
...
function GroupFighter:_addNpcGo_chilren(nListId, nW)	
	...
	for i=1,N do
		...
		if not child.szName then
			...
			if worldInfo.showName == 1 then
				child.szName = SimCityNPCInfo:generateName() or SimCityNPCInfo:getName(id)
			else
				child.szName = " "
			end
		...
		local _childLv = (child.level and child.level >= 1 and child.level <= 180) and child.level or (tbNpc.level or 95)
		child.level = _childLv
		nNpcIndex = AddNpcEx(child.nNpcId, _childLv, child.series or random(0,4), nMapIndex, targetPos[1] * 32, targetPos[2] * 32, 1, child.szName, 0)
		...
		if nNpcIndex > 0 then
			...
			if kind ~= 0 then 
				...
				if SetNpcLevel then SetNpcLevel(nNpcIndex, tbNpc.level or 1) end   -- [2026-06-25] ep level 95 (GroupFighter chua bao gio SetNpcLevel -> children m_Level=4 -> cast/skill-render sai, va dong bo voi parent)
				...
				if SimGear and SimGear.ApplyGearStats then
					SimGear:ApplyGearStats(child, nNpcIndex)
				elseif tbNpc.cap and NPCINFO_SetNpcCurrentLife then
					local maxHP = SimCityNPCInfo:getHPByCap(tbNpc.cap)
					NPCINFO_SetNpcCurrentMaxLife(nNpcIndex, maxHP)
					NPCINFO_SetNpcCurrentLife(nNpcIndex, maxHP)
