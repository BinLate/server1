    ...
    if nMapIndex >= 0 then
        ...
        if nNpcIndex > 0 then
            ...
            if kind ~= 0 then
                ...
                -- 100% Simbot Override Ngoai Trang & Thu Cuoi theo cap do thuc te
                if SimProgression and SimProgression.ApplyGearByLevel then
                    SimProgression:ApplyGearByLevel(tbNpc, nNpcIndex)
                elseif SimCityNgoaiTrang and SimCityNgoaiTrang.makeup then
                ...
                -- 100% Simbot Tinh Toan & Ghi De Trang Bi + Chi So C-Engine Thuc Te
                if SimGear and SimGear.ApplyGearStats then
                    SimGear:ApplyGearStats(tbNpc, nNpcIndex)
                else
                    local _curLv = tbNpc.level or 1
                    if SetNpcLevel then SetNpcLevel(nNpcIndex, _curLv) end
                    if SimProgression and SimProgression.CalcMaxHP then
                        tbNpc.maxHP = SimProgression:CalcMaxHP(_curLv, tbNpc.faction)
                    end
                    tbNpc.maxHP = tbNpc.maxHP or 2000
                    ...
                    if SetNpcAtkSpeed then SetNpcAtkSpeed(nNpcIndex, tbNpc.attackSpeed or 100) end
                end
