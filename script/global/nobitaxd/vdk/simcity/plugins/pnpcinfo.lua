...
function SimCityNPCInfo:generateName()
	if SimCityPlayerNames and getn(SimCityPlayerNames) > 0 then
		local _name = SimCityPlayerNames[random(1, getn(SimCityPlayerNames))]
		if _name and _name ~= "" and _name ~= "Name" then
			return _name
		end
	end
	return "DocCoCauBai"
